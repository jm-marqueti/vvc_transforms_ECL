...
def decimal_to_9bit_binary(decimal_number):
    if decimal_number < -255 or decimal_number > 255:
        raise ValueError("Decimal number is out of the 9-bit signed integer range")

    if decimal_number < 0:
        binary_number = bin(decimal_number & 0x1FF)[2:]
    else:
        binary_number = format(decimal_number, '09b')

    return binary_number

def decimal_to_20bit_binary(decimal_number):
    if decimal_number < -1050000  or decimal_number > 1050000:
        raise ValueError("Decimal number is out of the 20-bit signed integer range")

    if decimal_number < 0:
        binary_number = bin(decimal_number & 0xFFFFF)[2:]
        for j in range(0,20-len(binary_number)):
            binary_number = "1" + binary_number
    else:
        binary_number = format(decimal_number, '020b')
...
def DCT_2(N):
    ...
    for i in range(0, N):
        ...
        for j in range(0, N):
            ...
            t = round(t * 64)
            arredondamentos = {84: 83, -84: -83, 35: 36, -35: -36, 26: 25, -26: -25, 30: 31, -30: -31, 39: 38, -39: -38,
                               47: 46, -47: -46}
    ...
    for cases in range(0, blocks_wished):  # numero de matrizes geradas
       ...
       # print(x_bonito)
        ...
        for linha in transformed_med:
            new_linha = []
            for vetor in linha:
                vetor_bin = decimal_to_20bit_binary(vetor)[:16]  # binario -> binario truncado
                if (vetor_bin[0] == '1'):
